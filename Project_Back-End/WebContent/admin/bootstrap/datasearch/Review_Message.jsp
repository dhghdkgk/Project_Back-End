...
<c:choose>
	<c:when test="${WHERE eq 'INS' }">
		...
		<c:set var="SUC_URL" value="/Reviewofnowweek.do"/>
	</c:when>
	<c:when test="${WHERE eq 'EDT' }">
		<c:set var="SUC_MSG" value="수정 성공"/>
		<c:set var="FAIL_MSG" value="수정 실패"/>
		<c:set var="SUC_URL" value="/ReviewofnowweekOk.do?no=${no }&nowPage=${nowPage}"/>
	</c:when>
	<c:otherwise>
		<c:set var="SUC_MSG" value="삭제 성공"/>
		<c:set var="FAIL_MSG" value="삭제 실패"/>
		<c:set var="SUC_URL" value="/ReviewofnowweekEdit.do"/>
	</c:otherwise>
</c:choose>
